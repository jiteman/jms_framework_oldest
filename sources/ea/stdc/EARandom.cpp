...
{
    const uint32_t nRandNoLimit = RandomUint32Uniform();                                                    // Get value from [0, MAX_UINT32]
    #if defined(EA_PLATFORM_PS3) && defined(__SNC__)    // SNC has a code gen bug as of PS3 SDK 270.
        const uint32_t nReturnValue = (uint32_t)(((nRandNoLimit & 0xffffffff) * (uint64_t)nLimit) >> 32);   // Scales the value from [0, MAX_UINT32] to a range of [0, nLimit).
    #else
        const uint32_t nReturnValue = (uint32_t)((nRandNoLimit * (uint64_t)nLimit) >> 32);                  // Scales the value from [0, MAX_UINT32] to a range of [0, nLimit).
    #endif
    EA_ASSERT(nReturnValue < nLimit);
    return nReturnValue; 
}
...
double RandomLinearCongruential::RandomDoubleUniform()
{
    ...
    // If the seed is zero, the code fails to work because it 
    // will always return the same value.
    if(mnSeed == 0)
        mnSeed = (uint32_t)(EARandomGetCPUCycle() & 0xffffffff);
    mnSeed *= 0x278dde6d;
    ...
    // signed conversion.
    double dResult = (kDoubleUniformScaleFactor * (int32_t)mnSeed) + 0.5;

    #ifdef EA_PLATFORM_WIN32
        // You cannot rely on the open end condition if Direct3D is active,
        // since D3D kicks the FPU precision down to 24-bit, which then 
        // causes rounding up to the end value during scaling.
        if(dResult >= 1.0)
            dResult = 1.0;
    #endif
...
double RandomTaus::RandomDoubleUniform()
{
    ...
    if(dResult >= 1.0) // Due to precision issues, we need to clamp this.
        dResult = 1.0;
...
{
    register uint32_t* pState = &mState[0];
    register int i = kStateCount;
...
void RandomMersenneTwister::Reload()
{
    ...
    register uint32_t *p0 = &mState[0], *p2 = &mState[1], *pM = &mState[kPeriodValue];
    register uint32_t  s0 =  mState[0],  s1 =  mState[1];
    register int i;
