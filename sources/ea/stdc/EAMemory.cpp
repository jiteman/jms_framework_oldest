...
// The dcbz (data cache block zero) instruction initializes a data cache line.
// It does so without transferring data between registers and memory.
// It works as if were a C function declared as: 
//     void Clear128Bytes(void* pMem, size_t offset);
// The PS3 has 128 byte cache lines.


// MemclearDcbzPS3
// This function clears blocks of memory >= the cache line size via calls to 
// the dcbz instruction. The input address doesn't have alignment requirements.
// A general memclear function would call memset(p, 0, n) for sizes < a cache
// line and call this for greater sizes.

void MemclearDcbzPS3(void* pDestination, size_t n)
{
    EA_ASSERT(n >= kCacheLineSize); // Don't call this function unless this is true. The code below optimizes via this assumption.

    char* p = (char*)pDestination;
    char* p2;
    char* pEnd = p + n;

    if((uintptr_t)pDestination & kCacheLineSizeMask)  // If the start is not cache-line aligned...
    {
        // Clear any portion up to machine word alignment with bytes.
        p2 = (char*)(((uintptr_t)pDestination + kMachineWordSizeMask) & ~kMachineWordSizeMask);

        for(; p < p2; ++p)
            *p = 0;

        // Clear any portion up to cache line alignement with machine words.
        p2 = (char*)(((uintptr_t)pDestination + kCacheLineSizeMask) & ~kCacheLineSizeMask);
            
        for(; p < p2; p += kMachineWordSize) // To consider: we could get a little better performance by unrolling this loop.
            *((machine_word_t*)p) = 0;
    }


    // Set p2 to the floor cache line address <= pDestination + n.
    p2 = (char*)(((uintptr_t)pDestination + n) & ~kCacheLineSizeMask);

    // We would't gain much from unrolling this.
    for(; p < p2; p += kCacheLineSize)
        __dcbz(p);


    if((uintptr_t)pEnd & kCacheLineSizeMask)  // If the end is not cache-line aligned...
    {
        // Clear final portion up to the end with machine words.
        p2 = (char*)((uintptr_t)pEnd & ~kMachineWordSizeMask);

        for(; p < p2; p += kMachineWordSize) // To consider: we could get a little better performance by unrolling this loop.
            *((machine_word_t*)p) = 0;

        // Clear final portion smaller than a cache line with bytes.
        for(; p < pEnd; ++p)
            *p = 0;
    }
}
...
void MemclearDcbzWii(void* pDestination, size_t n);  // Wii compiler wants a prototype.
