...
//    
//    uint32_t DJB2         (const void*     pData, size_t nLength, uint32_t nInitialValue = kDJB2InitialValue);
//    uint32_t DJB2_String8 (const char8_t*  pData, uint32_t nInitialValue = kDJB2InitialValue, CharCase charCase = kCharCaseAny);
//    uint32_t DJB2_String16(const char16_t* pData, uint32_t nInitialValue = kDJB2InitialValue, CharCase charCase = kCharCaseAny);
//    
...
namespace StdC
{
    ...
    /// Classification:
    ///   String hash
    ///   DJB2 is not a cryptographic-level hash.
    ///
    /// Description:
    ///   DJB2 is a very quick string hashing function that yields acceptable
    ///   collision and dispersion rates for many cases. These are very fast, 
    ///   as the algorithm uses one shift and one add per character. 
    ///   The quality of the hashing is good enough for many uses. It can be 
    ///   faster than FNV1, but generates more collisions than FNV1.
    ///
    /// Notes:
    ///   This string hash hashes the characters, not the binary bytes. 
    ///   As a result, the hash of "hello" and EA_CHAR16("hello") will yield the 
  ...
  //EASTDC_API uint32_t DJB2_String32(const char32_t* pData, uint32_t nInitialValue = kDJB2InitialValue, CharCase charCase = kCharCaseAny);
