...
//
#if !EA_THREADS_AVAILABLE             || \
    defined(EA_PLATFORM_PLAYSTATION2) || \
    defined(EA_PLATFORM_GAMECUBE)     || \
    defined(EA_PLATFORM_REVOLUTION)   || \
    defined(EA_PLATFORM_CTR)          || \
    defined(EA_PLATFORM_MAC)          || \
...
namespace EA
{
    ...
    namespace Thread
    {
        ...
        #elif defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_REVOLUTION) || defined(EA_PLATFORM_CTR)
            #define EATHREAD_SCHED    kSchedulingFIFO
        ...
        #elif defined(EA_PLATFORM_PS3_SPU)
            #define EATHREAD_SCHED    kSchedulingCoop
        ...
        // be specified in finer units, such as microseconds.
        ...
        #elif (defined(EA_PLATFORM_PS3) || defined(EA_PLATFORM_PS3_SPU)) && EA_THREADS_AVAILABLE
            typedef double     ThreadTime;                        /// Double allows the user to specify sub-millisecond level thread times.
            ...
            const   ThreadTime kTimeoutNone      = DBL_MAX;       /// Used to specify to functions to block without a timeout (i.e. block forever).
            const   ThreadTime kTimeoutYield     = 0.001;         /// This is used with ThreadSleep to minimally yield to threads of equivalent priority.

            #define EA_THREADTIME_AS_INT64(t)  ((int64_t)(t))     /// Returns the time as int64_t milliseconds. The PS3 recognizes thread time as microseconds, so this function can't represent full precision. EA_THREADTIME_AS_DOUBLE can do so.
            #define EA_THREADTIME_AS_DOUBLE(t) ((double)(t))      /// Returns the time as double milliseconds. May include a fraction component.

        #elif defined(EA_PLATFORM_REVOLUTION) && EA_THREADS_AVAILABLE
            ...
            #define EA_THREADTIME_AS_DOUBLE(t) ((double)(t))
        #elif defined(EA_PLATFORM_PSP2)
            // PSP2 treats most timeout values as microseconds.
            // We might want to convert ThreadTime to double (as in PS3), in order to give
            // the user more precision
            typedef uint64_t ThreadTime;
            const ThreadTime kTimeoutImmediate = 0;
            const ThreadTime kTimeoutNone = UINT64_MAX;
            const ThreadTime kTimeoutYield = 0;

            #define EA_THREADTIME_AS_INT64(t) (t)
            #define EA_THREADTIME_AS_DOUBLE(t) ((double)(t))
        ...
        // item is of significance because Sony platforms (e.g. PS2, PS3) natively define 
        // lower integers to mean higher thread priorities. With EAThread you get consistent
        // behaviour across platforms and thus kThreadPriorityDefault + 1 always results in 
        // a thread that runs at priority of one level higher. On Sony platforms, this + 1
        // gets translated to a - 1 when calling the Sony native thread priority API.
        // EAThread priorities have no mandated integral bounds, though kThreadPriorityMin and 
        // kThreadPriorityMax are defined as convenient practical endpoints for users.
        // Users should not generally use hard-coded constants to refer to EAThread priorities
        // much like it's best not to use hard-coded constants to refer to platform-specific
        // native thread priorities. Also, users generally want to avoid manipulating thread
        // priorities to the extent possible and use conventional synchronization primitives
        // to control execution. Similarly, wildly varying thread priorities such as +100 are 
        // not likely to achieve much and are not likely to be very portable.
        //
        ...
        #elif defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_REVOLUTION) || defined(EA_PLATFORM_CTR)
            const int kSysThreadPriorityDefault = 16;
        #elif defined(EA_PLATFORM_PS3)
            const int kSysThreadPriorityDefault = 1001;
        #elif defined(EA_PLATFORM_PS3_SPU)
            const int kSysThreadPriorityDefault = 1001;
        #elif defined(EA_PLATFORM_PS2)
            const int kSysThreadPriorityDefault = 64;   
        #elif defined(EA_PLATFORM_PSP2)
		    #if	(SCE_PSP2_SDK_VERSION >= 0x00940050) 
                // The value is set to 160 because in PSP2 SDK 0.945 SCE_KERNEL_DEFAULT_PRIORITY_USER was  
                // changed so that it was the priority of the process. The process priority defaults to 160.
                const int kSysThreadPriorityDefault = 160;
		    #else
			    const int kSysThreadPriorityDefault = SCE_KERNEL_DEFAULT_PRIORITY_USER;
		    #endif
        #else
        ...
        const int kProcessorDefault = -1;    /// Use the default processor for the platform. On many platforms, the default is to not be tied to any specific processor, but on others threads can only ever be bound to a single processor.
        ...
        EATHREADLIB_API void AssertionFailureV(const char* pFormat, ...);
             ...
             /// Calculates a String Hash
             /// Usually used in platform where it expects an unique number rather than 
             /// a string as a resource descriptor
             EATHREADLIB_API uint32_t FNVStringHash(const char* str);
        #endif
...
///
    ...
    //    // Might be a way to get the loadwordupdate intrinsic to work here.
    //    // We can't use a technique like this unless we know it is portable 
    //    // between sdk versions. Magic numbers like below are likely not.
    //    #define EAThreadGetUniqueId(dest) 
    //        __asm lwz r8, 256(r13)
    //        __asm lwz r9, 332(r8)
    //        __asm stw r9, dest
    //#else
        // We can probably use a __readfsdword-based implementation for XBox 360.
        // The XENON (XBox 360) platform uses register 13 to store thread data, but it
        // doesn't make r13 unique among threads; it makes what r13 points to unique 
        // among threads, but we don't have anything we can rely on within there as 
        // being constant across SDK versions.
        extern "C" unsigned long __stdcall GetCurrentThreadId();
        #define EAThreadGetUniqueId(dest) dest = (EA::Thread::ThreadUniqueId)(uintptr_t)GetCurrentThreadId()
    //#endif

#elif defined(EA_PLATFORM_PS3)

    // Reference implementation:
    //#define EAThreadGetUniqueId(dest) sys_ppu_thread_get_id(&dest);

    #if defined(__SNC__)
        #define EAThreadGetUniqueId(dest) dest = (EA::Thread::ThreadUniqueId)__reg(13);
    #else
        // Fast implementation:
        inline uint32_t EAFutexGetR13() { 
            register uint32_t x;
            __asm__ __volatile__ ("mr %0, 13" : "=r" (x) );
            return x;
        }
        #define EAThreadGetUniqueId(dest) dest = EAFutexGetR13()
    #endif

#elif defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_REVOLUTION) || defined(EA_PLATFORM_CTR)

    // On this platform, GetThreadId returns OSThread*, and CodeWarrior doesn't like 
    // to convert OSThread* to uintptr_t witout casting it to void* first.
    #define EAThreadGetUniqueId(dest) dest = (EA::Thread::ThreadUniqueId)(uintptr_t)(void*)EA::Thread::GetThreadId()

#elif defined(EA_PLATFORM_PSP2)
	// Directly call the system call
	extern "C" int sceKernelGetThreadId(void);
	#define EAThreadGetUniqueId(dest) dest = (EA::Thread::ThreadUniqueId)sceKernelGetThreadId()

    // This is currently as of SDK 940 equivalent to 
	//	#define EAThreadGetUniqueId(dest) dest = (EA::Thread::ThreadUniqueId) ((uint32_t *)__builtin_mrc(15, 0, 13, 0, 3))[-511];

	// Can we do this without the indirect load?
	//	#define EAThreadGetUniqueId(dest) dest = (EA::Thread::ThreadUniqueId) __builtin_mrc(15, 0, 13, 0, 3);

// Disabled until we can have it be savvy to earlier Mac and iPhone OS versions:
//#elif defined(EA_PLATFORM_APPLE)
//
//    inline void EAThreadGetUniqueId(EA::Thread::ThreadUniqueId& uniqueId)
//    {
//        // Must do: Make EA::Thread::ThreadUniqueId be uint64_t for this platform.
//        // pthread_threadid_np -> __OSX_AVAILABLE_STARTING(__MAC_10_6, __IPHONE_3_2);
//        uint64_t uniqueId64; pthread_threadid_np(pthread_self(), &uniqueId64); uniqueId = (EA::Thread::ThreadUniqueId)uniqueId64;
//    }
