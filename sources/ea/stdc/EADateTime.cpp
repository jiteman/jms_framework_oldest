...
#include "EA/StdC/EADateTime.h"
#include "EA/StdC/EAString.h"
#include "EA/StdC/EASprintf.h"
#include "EA/StdC/EACType.h"
#include "EA/StdC/EAStopwatch.h"
#include "EA/StdC/EAMemory.h"
#include <float.h>
#include <math.h>
#include <string.h>
...
namespace StdC
{
    ...
    #define EADT_COUNT_LEAP_YEARS(Y) (((Y - 1) / 4) - ((Y - 1) / 100) + ((Y - 1) / 400))
        ...
        ///////////////////////////////////////////////////////////////////////
        // GetTimeTFromCtrDateTime
        //
        // Given a CTR DateTime object, returns the equivalent time_t value.
        //
        static time_t GetTimeTFromCtrDateTime(const ::nn::fnd::DateTime& datetime)
        {
            tm tmCurrent;
            memset(&tmCurrent, 0, sizeof(tmCurrent));
            tmCurrent.tm_year = datetime.GetYear() - 1900;
            tmCurrent.tm_mon = datetime.GetMonth() - 1;
            tmCurrent.tm_mday = datetime.GetDay();
            tmCurrent.tm_hour = datetime.GetHour();
            tmCurrent.tm_min = datetime.GetMinute();
            tmCurrent.tm_sec = datetime.GetSecond();
            return mktime(&tmCurrent);
        }
    #endif
    ...
    */
    ...
    {
        #if defined(EA_PLATFORM_PS3)
            return 1;                           // Nanosecond precision. In practice it's surely a little worse.
        ...
        #elif defined(EA_PLATFORM_AIRPLAY)      // Airplay is a mobile middleware package which acts like a generic platform.
            return 1000;                        // Microsecond precision.
            ...
            return UINT64_C(1000000000);        // Second-level precision.  The GetTimeOfDay currently sets microseconds to 0
    ...
    void DateTime::Set(TimeFrame timeFrame, bool bSetNanoseconds)
    {
            ...
            // We use this on Wii because the Wii C time functions appear to be broken.
            OSCalendarTime ct;
            ...
            Set((uint32_t)ct.year, (uint32_t)ct.mon + kMonthJanuary, (uint32_t)ct.mday, (uint32_t)ct.hour, (uint32_t)ct.min, (uint32_t)ct.sec, (uint32_t)(tv.tv_usec * 1000));

        #elif defined(EA_PLATFORM_CTR)
            ::nn::fnd::DateTime pTime = ::nn::fnd::DateTime::GetNow();
            ...
            Set(pTime.GetYear(), pTime.GetMonth(), pTime.GetDay(), pTime.GetHour(), pTime.GetMinute(), pTime.GetSecond(), (uint32_t)(tv.tv_usec * 1000));
    ...
    {
        return (EA::StdC::int128_t(mnSeconds) * INT32_C(1000000000)) + mnNanosecond; // CodeWarrior mistakenly requires the INT32_C wrapper.
    }


    void DateTime::SetNanoseconds(const EA::StdC::int128_t& nanoseconds)
    {
        EA::StdC::int128_t seconds    = (nanoseconds / INT32_C(1000000000));
        EA::StdC::int128_t nanosecond = (nanoseconds % INT32_C(1000000000));
    ...
    {
        #if defined(EA_PLATFORM_CTR)
            time_t nTime = GetTimeTFromCtrDateTime(::nn::fnd::DateTime::GetNow());
        #else
    ...
    {
        #if defined(EA_PLATFORM_MICROSOFT) && !defined(EA_PLATFORM_XENON) && defined(_MSC_VER) && (_MSC_VER >= 1400)
            ...
            EA_UNUSED(bDaylightSavingsName);
                ...
                Snprintf(pName, kTimeZoneNameCapacity, "+%6d", b);
            #else
    ...
    EASTDC_API int GetTimeOfDay(timeval* pTV, timezone_* pTZ, bool bUTC)
    {
        ...
        if(!pTZ)
            ...
            return 0;
            ...
            #if EASTDC_PS3_LIBRTC_ENABLED // If we should use the better functionality present in librtc...
                // User needs to call on startup: cellSysmoduleLoadModule(CELL_SYSMODULE_RTC);
                // User needs to link app with:   librtc_stub.a

                // As of SDK 240, there doesn't seem to be a direct function for getting the time zone.
                // So we indirectly get it via some other functions.

                CellRtcTick tickLocal = { 1000000 } ;  // This is in terms of microseconds.
                CellRtcTick tickUTC   = { 1000000 };
                cellRtcConvertLocalTimeToUtc(&tickLocal, &tickUTC);

                pTZ->tz_minuteswest = (tickUTC.tick - tickLocal.tick) / (60000000); // Convert from microseconds to minutes.
                pTZ->tz_dsttime     = 0;    // PS3 doesn't seem to have a way to tell if daylight savings time is active.
                ...
                {     
                    ...
                    if(bUTC)
                        cellRtcGetCurrentClock(&dateTime, 0);
                    else
                    ...
                    pTV->tv_sec  = t;
                    pTV->tv_usec = dateTime.microsecond; // To do: Need to make sure it's valid to just use this value as-is.
                }
                ...
                pTZ->tz_dsttime     = 0; // Not available.
            ...
            // The Wii doesn't have the concept of universal time (UTC). All time identifiers are in local time.
            // We have no way of knowing where timezone-wise the machine is. So we have little choice but to 
            // pretend that all machines are in the same place (Switzerland / UTC).
            ...
            if(pTV)
            {
                ...
                // There are 31,556,926 seconds in a non-leap year.
                // There are 31,643,326 seconds in a leap year.
                // But there are also leap seconds, and skipped leap years.

                OSTime         osTime = OSGetTime();
                OSCalendarTime osCalendarTime; OSTicksToCalendarTime(osTime, &osCalendarTime);

                // We assume this function is being called in 2008 or later, so we precalculate the seconds
                // up to 2007 and dynamically calculate the seconds after that.
                pTV->tv_sec = (29 * 31556926) + (9 * 31643326); // Seconds from beginning of 1970 to beginning of 2008.

                for(int y = 2009; y < osCalendarTime.year; ++y)
                {
                    if(((y % 4 == 0) && (y % 100 != 0)) || (y % 400 == 0))
                        pTV->tv_sec += 31643326; // Leap
                    else
                        pTV->tv_sec += 31556926; // Non-leap
                }

                pTV->tv_sec  += (osCalendarTime.yday * 86400);
                pTV->tv_sec  += (osCalendarTime.hour *  3600);
                pTV->tv_sec  += (osCalendarTime.min  *    60);
                pTV->tv_sec  += (osCalendarTime.sec  *     1);

                pTV->tv_usec  = (osCalendarTime.msec  * 1000);
                pTV->tv_usec += (osCalendarTime.msec  *    1);
            ...
            if(pTV)
            {
                ::nn::fnd::DateTime dtCurrent = ::nn::fnd::DateTime::GetNow();
                pTV->tv_sec = GetTimeTFromCtrDateTime(dtCurrent);
                pTV->tv_usec = dtCurrent.GetMilliSecond() * 1000;
            }
