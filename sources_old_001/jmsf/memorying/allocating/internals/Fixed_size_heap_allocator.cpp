...
#include "jmsf/assisting/Find.hin"
...
// virtuals Allocator
Memory_pointer Fixed_size_heap_allocator::allocate_memory( const typeing::Memory_natural a_size, const typeing::Memory_natural an_alignment ) {
...
// Eisha: next to do
void Fixed_size_heap_allocator::deallocate_memory( const Memory_pointer &/*a_memory*/, const typeing::Memory_natural /*an_alignment*/ ) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Fixed_size_heap_allocator::Fixed_size_heap_allocator( const Memory_pointer &memory_heap, const typeing::Memory_natural heap_size )
	:
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Memory_pointer Fixed_size_heap_allocator::tryToAllocateMemory( const typeing::Memory_natural size_to_allocate, const typeing::Memory_natural an_alignment ) noexcept {
	Memory_pointer allocated_memory;
	...
	structing::traversing::List_traverser< Memory_block_descriptor > first_fit_memory_block =
		assisting::find_first_not(
			Memory_block_descriptor( typeing::Memory_natural_nill, an_alignment, size_to_allocate ),
			_empty_memory_list_sorted_by_size.create_traverser_at_begin(),
			comparing::Mbd_size_fit_comparator(),
			typeing::Comparasion_result::create_greater() );
...
void Fixed_size_heap_allocator::consolidate_and_add( const Memory_block_descriptor &new_empty_memory_block ) noexcept {
	structing::traversing::List_traverser< Memory_block_descriptor > block_above =
		assisting::find_first(
			...
			_empty_memory_list_sorted_by_address.create_traverser_at_begin(),
			comparing::Mbd_address_comparator(),
			typeing::Comparasion_result::create_lower() );
