...
// eathread_pool.cpp
//
// Copyright (c) 2003 - 2007, Electronic Arts Inc. All rights reserved.
//
...
#include <new>
    ...
    #pragma diag_suppress=1772 // argument of type "int" is implicitly converted to parameter
...
bool EA::Thread::ThreadPool::Init(const ThreadPoolParameters* pThreadPoolParameters)
{
    if(!mbInitialized)
    {
        if(pThreadPoolParameters && (mnCurrentCount == 0))
        {
            ...
            mDefaultThreadParameters.mnProcessor = -1;    // We control this separately.
            mDefaultThreadParameters.mbSuspended = false; // We control this separately.
...
intptr_t EA::Thread::ThreadPool::ThreadFunction(void* pContext)
{
    ...
    while(!pThreadInfo->mbQuit)
    {
        ...
        else
        {
            ...
            const Condition::Result result = pCondition->Wait(pMutex, GetThreadTime() + pThreadPool->mnIdleTimeoutMilliseconds);
...
{
    if(tp.mnProcessor == -1) // If we are to manipulate tp.mnProcessor...
    {
