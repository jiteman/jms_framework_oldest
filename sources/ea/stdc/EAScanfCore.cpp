...
// EASTDC_DOUBLE_NEGATIVE_0_FLAW
//
// Defined as 0 or 1. 
// This is to indicate that the compiler will not correctly 
// distinguish between the numeric constants -0.0 and 0.0.
//
#if !defined(EASTDC_DOUBLE_NEGATIVE_0_FLAW)
    #if defined(EA_PLATFORM_XENON) // XBox 360 handles -0.0 incorrectly, at least with our compiler settings.
        #define EASTDC_DOUBLE_NEGATIVE_0_FLAW 1
    #else
        #define EASTDC_DOUBLE_NEGATIVE_0_FLAW 0
    #endif
#endif
...
int FILEReader8(ReadAction readAction, int value, void* pContext)
{
    ...
    switch(readAction)
    {
        ...
        {
            #if (!defined(__GNUC__) || (__GNUC__ >= 4)) && !defined(EA_PLATFORM_AIRPLAY) // Older versions of GCC don't support fwide().
                      ...
                      int& nReadCount, int& bNegative, int& bIntegerOverflow)
{
    ...
    if((nBase != 1) && (nBase <= 36) && (nMaxFieldWidth >= 1))
    {
        ...
        c = pReadFunction8(kReadActionRead, 0, pContext);
        nReadCount++;
            ...
            switch ((int)state)
            {
                ...
                case kRISReadDigits:
                {
                    ...
                    else  //Else we may have a hex digit, but we only pay attention to it if it's compatible with nBase (e.g. base 16).
                    {
                        ...
                        if((nBase > 10) && ((cLower = Tolower((char8_t)c)) >= 'a') && ((cHex = (10 + (int)cLower - 'a')) < nBase))
                            c = cHex;
                        else
                        {
                            if(state == kRISReadDigits)
        ...
        pReadFunction8(kReadActionUnread, c, pContext);
                       ...
                       int& nReadCount, int& bNegative, int& bIntegerOverflow)
{
    ...
    if((nBase != 1) && (nBase <= 36) && (nMaxFieldWidth >= 1))
    {
        ...
        c = pReadFunction16(kReadActionRead, 0, pContext);
        nReadCount++;
            ...
            switch ((int)state)
            {
                ...
                case kRISReadDigits:
                {
                    ...
                    else  //Else we may have a hex digit, but we only pay attention to it if it's compatible with nBase (e.g. base 16).
                    {
                        ...
                        // If the base is > 10 and if c is a char that can represent a digit in the base...
                        if((nBase > 10) && ((cLower = Tolower((char16_t)c)) >= 'a') && ((cHex = (10 + (int)cLower - 'a')) < nBase))
                            c = cHex;
                        else
                        {
                            if(state == kRISReadDigits)
        ...
        // Return the final char back to the stream. It will typically be a 0 (nul terminator) char.
        pReadFunction16(kReadActionUnread, c, pContext);
                       ...
                       int& nReadCount, int& bNegative, int& bIntegerOverflow)
{
    ...
    if((nBase != 1) && (nBase <= 36) && (nMaxFieldWidth >= 1))
    {
        ...
        c = pReadFunction32(kReadActionRead, 0, pContext);
        nReadCount++;
            ...
            switch ((int)state)
            {
                ...
                case kRISReadDigits:
                {
                    ...
                    else  //Else we may have a hex digit, but we only pay attention to it if it's compatible with nBase (e.g. base 16).
                    {
                        ...
                        // If the base is > 10 and if c is a char that can represent a digit in the base...
                        if((nBase > 10) && ((cLower = Tolower((char32_t)c)) >= 'a') && ((cHex = (10 + (int)cLower - 'a')) < nBase))
                            c = cHex;
                        else
                        {
                            if(state == kRISReadDigits)
        ...
        // Return the final char back to the stream. It will typically be a 0 (nul terminator) char.
        pReadFunction32(kReadActionUnread, c, pContext);
                   ...
                   int nMaxFieldWidth, int cDecimalPoint, int& nReadCount, int& bOverflow)
{
    ...
    while((c != kReadError) && (nFieldCount <= nMaxFieldWidth) && !(state & kRDSDone)) 
    {
        switch((int)state)
        {
            ...
            case kRDSInfinity:
            {
                ...
                while((i < 8) && ((int)Toupper((char8_t)c) == (int)"INFINITY"[i]))
                {
                    i++;
                    c = pReadFunction8(kReadActionRead, 0, pContext);
                    nFieldCount++;
            ...
            case kRDSNAN:
            {
                ...
                int     j = 0;
                //char8_t pNANString[24]; pNANString[0] = 0;

                while((i < 4) && ((int)Toupper((char8_t)c) == (int)"NAN("[i]))
                {
                    c = pReadFunction8(kReadActionRead, 0, pContext);
                    nFieldCount++;
                    i++;
                ...
                if((i == 3) || (i == 4))
                {
                    if(i == 4)
                    {
                        while((j < 32) && (Isdigit((char8_t)c) || Isalpha((char8_t)c)))
                        {
                            //pNANString[j] = (char8_t)c;
                            c = pReadFunction8(kReadActionRead, 0, pContext);
                            nFieldCount++;
                            j++;
                        }
    ...
    char8_t* p = doubleValue.mSigStr + i;
    ...
    else
    {
        ...
        // There is a potential compiler has a bug where it will not differentiate
        // between -0.0 and 0.0, and thus will optimize this conditional statement
        // away, instead returning the first value regardless of bNegative's value.
        // volatile is neccessary to remove the optimization.
        #if EASTDC_DOUBLE_NEGATIVE_0_FLAW
            static volatile const double dNegZero = -0.0;
            static volatile const double dPosZero =  0.0;

            return bNegative ? dNegZero : dPosZero;
        #else
    ...
    while((c != kReadError) && (nFieldCount <= nMaxFieldWidth) && !(state & kRDSDone)) 
    {
        switch((int)state)
        {
            ...
            case kRDSInfinity:
            {
                ...
                while((i < 8) && ((int)Toupper((char16_t)c) == (int)"INFINITY"[i]))
                {
                    i++;
                    c = pReadFunction16(kReadActionRead, 0, pContext);
                    nFieldCount++;
            ...
            case kRDSNAN:
            {
                ...
                int      i = 1; // We have already matched the first 'N' char.
                int      j = 0;
                //char16_t pNANString[24]; pNANString[0] = 0;

                while((i < 4) && ((int)Toupper((char16_t)c) == (int)"NAN("[i]))
                {
                    c = pReadFunction16(kReadActionRead, 0, pContext);
                    nFieldCount++;
                    i++;
                ...
                if((i == 3) || (i == 4))
                {
                    if(i == 4)
                    {
                        while((j < 32) && (Isdigit((char16_t)c) || Isalpha((char16_t)c)))
                        {
    ...
    while((c != kReadError) && (nFieldCount <= nMaxFieldWidth) && !(state & kRDSDone)) 
    {
        switch((int)state)
        {
            ...
            case kRDSInfinity:
            {
                ...
                while((i < 8) && ((int)Toupper((char32_t)c) == (int)"INFINITY"[i]))
                {
                    i++;
                    c = pReadFunction32(kReadActionRead, 0, pContext);
                    nFieldCount++;
            ...
            case kRDSNAN:
            {
                ...
                int      i = 1; // We have already matched the first 'N' char.
                int      j = 0;
                //char32_t pNANString[24]; pNANString[0] = 0;

                while((i < 4) && ((int)Toupper((char32_t)c) == (int)"NAN("[i]))
                {
                    c = pReadFunction32(kReadActionRead, 0, pContext);
                    nFieldCount++;
                    i++;
                ...
                if((i == 3) || (i == 4))
                {
                    if(i == 4)
                    {
                        while((j < 32) && (Isdigit((char32_t)c) || Isalpha((char32_t)c)))
                        {
                            //pNANString[j] = (char32_t)c;
                            c = pReadFunction32(kReadActionRead, 0, pContext);
                            nFieldCount++;
                            j++;
                        }
...
const char8_t* ReadFormat8(const char8_t* pFormat, ScanfLocal::FormatData* pFormatData)
{
    ...
    switch (c)
    {
        ...
        case 'S': // We accept %hS, %S, %lS, %I8s, %I16s, %I32s (regular, wide,    wide, char8_t, char16_t, char32_t)
        {
            ...
            {
                fd.mnType = kFormatError;
                EA_FAIL_MSG("Scanf: Invalid %c/%s modifier");
            }
        ...
        case '[':
        {
            ...
            {
                #if EASCANF_MS_STYLE_S_FORMAT
                    fd.mModifier = kModifierChar;  // This is the same as directly below.
                #else
                    fd.mModifier = kModifierChar;
                #endif
            }
        ...
        case 'n':
        {
            ...
            // Execution of a %n conversion specification shall not increment the assignment count returned 
...
const char16_t* ReadFormat16(const char16_t* pFormat, ScanfLocal::FormatData* pFormatData)
{
    ...
    switch (c)
    {
        ...
        case '[':
        {
            ...
            {
                #if EASCANF_MS_STYLE_S_FORMAT
                    fd.mModifier = kModifierWChar; // This is the same as directly below.
                #else
                    fd.mModifier = kModifierWChar;
                #endif
            }
            ...
            if(c == ']') // The C99 standard requires that if there is an excluded ']' char, then it is the first char after [ or [^.
            {
                fd.mCharBitmap.Set16(']');
                c = *++pFormatCurrent;
            }

            while(c && (c != ']'))  // Walk through the characters until we encounter a closing ']' char. Use '-' char to indicate character ranges, as in "a-d"
            {
                fd.mCharBitmap.Set16(c);
...
const char32_t* ReadFormat32(const char32_t* pFormat, ScanfLocal::FormatData* pFormatData)
{
    ...
    switch (c)
    {
        ...
        case '[':
        {
            ...
            if(c == ']') // The C99 standard requires that if there is an excluded ']' char, then it is the first char after [ or [^.
            {
                fd.mCharBitmap.Set32(']');
                c = *++pFormatCurrent;
            }

            while(c && (c != ']'))  // Walk through the characters until we encounter a closing ']' char. Use '-' char to indicate character ranges, as in "a-d"
            {
                fd.mCharBitmap.Set32(c);
...
int VscanfCore(ReadFunction8 pReadFunction8, void* pContext, const char8_t* pFormat, va_list arguments)   
{
    ...
    const char8_t*      pFormatCurrent = pFormat;   // Current position within entire fd string.
    ...
    pReadFunction8(kReadActionBegin, sizeof(*pFormat), pContext);

    while(*pFormatCurrent)
    {
        char8_t cFormat = *pFormatCurrent;
        ...
        if(Isspace(cFormat))
        {
            ...
            while(Isspace((char8_t)(c = pReadFunction8(kReadActionRead, 0, pContext))))
                ++nReadCountSum;
        ...
        if(cFormat != '%')
        {
            if((c = pReadFunction8(kReadActionRead, 0, pContext)) != (int)cFormat)
            {
                pReadFunction8(kReadActionUnread, c, pContext);
                goto Done;
            }
        ...
        pFormatCurrent = ReadFormat8(pFormatCurrent, &fd);
        ...
        if((fd.mnType != 'n') && (pReadFunction8(kReadActionGetLastError, 0, pContext) || pReadFunction8(kReadActionGetAtEnd, 0, pContext)))
            break;
        ...
        switch (fd.mnType)
        {
            ...
            case 'X':
            {
                ...
                switch (fd.mModifier)
                {
                    ...
                    case kModifierLongLong:
                        ullValue = (unsigned long long)ReadUint64_8(pReadFunction8, pContext, UINT64_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;

                    case kModifierInt32:
                    case kModifierLong:
                        ulValue  = (unsigned long)     ReadUint64_8(pReadFunction8, pContext, UINT32_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;

                    case kModifierInt16:
                    case kModifierShort:
                        ulValue  = (unsigned long)     ReadUint64_8(pReadFunction8, pContext, UINT16_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;

                    case kModifierInt8:
                    case kModifierChar:
                    default:
                        ulValue  = (unsigned long)     ReadUint64_8(pReadFunction8, pContext, UINT8_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;
                }
                ...
                if((fd.mnType == 'i' || fd.mnType == 'd')) // If using a signed type...
                {
                    if(bNegative)
                    {
                        if((fd.mModifier == kModifierLongLong) || (fd.mModifier == kModifierInt64))
                          #ifdef EA_PLATFORM_PLAYSTATION2
                            llValue = 0;  // PS2 doesn't support negating long long. Note that long long on PS2 is 128 bits and not 64 bits. So in our implementation here %I64 doesn't work on PS2.
                          #else
                            llValue = -(long long)ullValue;
                          #endif
                        else
                            lValue  = -(long)ulValue;
                    }
                    ...
                    if(pArgumentCurrent) // If we should write the value to a user argument...
                    {
                        switch (fd.mModifier)
                        {
                            case kModifierInt64:    *(int64_t*)   pArgumentCurrent = (int64_t)   llValue; break;
                            case kModifierLongLong: *(long long*) pArgumentCurrent =             llValue; break;
                            case kModifierInt32:    *(int32_t*)   pArgumentCurrent = (int32_t)   lValue;  break; // We assume sizeof long >= sizeof int32
                            case kModifierLong:     *(long*)      pArgumentCurrent = (long)      lValue;  break;
                            case kModifierInt16: 
                            case kModifierShort:    *(short*)     pArgumentCurrent = (short)     lValue;  break;
                            case kModifierInt8:  
                            case kModifierChar:     *(char*)      pArgumentCurrent = (char)      lValue;  break;
                            case kModifierNone:     *(int*)       pArgumentCurrent = (int)       lValue;  break;
                ...
                else
                {
                    if(bNegative)
                    {
                        ...
                        if((fd.mModifier == kModifierLongLong) || (fd.mModifier == kModifierInt64))
                          #ifdef EA_PLATFORM_PLAYSTATION2
                            ullValue = 0;  // PS2 doesn't support negating long long. Note that long long on PS2 is 128 bits and not 64 bits. So in our implementation here %I64 doesn't work on PS2.
                          #else
                            ullValue = (unsigned long long)-(long long)ullValue;
                          #endif
                        else
                            ulValue  = (unsigned long)-(long)ulValue;
                    }
                    // else leave as-is.
                    ...
                    if(pArgumentCurrent) // If we should write the value to a user argument...
                    {
                        switch (fd.mModifier)
                        {
                            case kModifierInt64:    *(uint64_t*)           pArgumentCurrent = (uint64_t)           ullValue; break;
                            case kModifierLongLong: *(unsigned long long*) pArgumentCurrent =                      ullValue; break;
                            case kModifierInt32:    *(uint32_t*)           pArgumentCurrent = (uint32_t)           ulValue;  break; // We assume sizeof ulong >= sizeof uint32
                            case kModifierLong:     *(unsigned long*)      pArgumentCurrent = (unsigned long)      ulValue;  break;
                            case kModifierInt16: 
                            case kModifierShort:    *(unsigned short*)     pArgumentCurrent = (unsigned short)     ulValue;  break;
                            case kModifierInt8:  
                            case kModifierChar:     *(unsigned char*)      pArgumentCurrent = (unsigned char)      ulValue;  break;
                            case kModifierNone:     *(unsigned int*)       pArgumentCurrent = (unsigned int)       ulValue;  break;
                            default: /* This should never occur. Possibly assert false */                                    break;
                        }
            ...
            case 'A':
            {
                ldValue = ReadDouble8(pReadFunction8, pContext, fd.mnWidth, fd.mDecimalPoint, nReadCount, bIntegerOverflow);
            ...
            case '[':
            {
                ...
                if(pArgumentCurrent) // If we should write the value to a user argument...
                {
                    ...
                    while(fd.mnWidth-- && ((c = pReadFunction8(kReadActionRead, 0, pContext)) != -1) && fd.mCharBitmap.Get8(c)) 
                    {
                ...
                if(pArgumentCurrent) // If we should write the value to a user argument...
                {
                    while(fd.mnWidth-- && ((c = pReadFunction8(kReadActionRead, 0, pContext)) != -1) && fd.mCharBitmap.Get8(c)) 
                    {
                        if(fd.mModifier == kModifierWChar)
                        {
                            if((unsigned char)(char)c < 128)
                                *((char16_t*)pArgumentCurrent) = (unsigned char)(char)c;
                            else
                            {
                                // We need to convert from UTF8 to UCS2 here. However, this can be complicated because 
                                // multiple UTF8 chars may correspond to a single UCS2 char. Luckily, the UTF8 format allows us
                                // to know how many chars are in a multi-byte sequence based on the char value.
                                char8_t      buffer[7];
                                const size_t utf8Len   = utf8lengthTable[(unsigned)c];
                                char16_t     c16[2];
                        ...
                        {
                            *((char8_t*)pArgumentCurrent) = (char8_t)c;
                            ++pArgumentCurrent;
                        }
            ...
            case 'C': // Actually %C is not a standard scanf format.
            {
                ...
                if(pArgumentCurrent) // If we should write the value to a user argument...
                {
                    ...
                    while(fd.mnWidth-- && ((c = pReadFunction8(kReadActionRead, 0, pContext)) != -1))
                    {
                        switch (charTypeSize)
                        {
                            case 1:
    ...
    Done:
    if((nConversionCount == 0) && pReadFunction8(kReadActionGetLastError, 0, pContext))
        nAssignmentCount = -1;

    pReadFunction8(kReadActionEnd, 0, pContext);
...
int VscanfCore(ReadFunction16 pReadFunction16, void* pContext, const char16_t* pFormat, va_list arguments)   
{
    ...
    int                 nReadCount;                 // Temporary holder.
    int                 nReadCountSum = 0;          // Used to support the %n field.
    const char16_t*     pFormatCurrent = pFormat;   // Current position within entire fd string.
    char*               pArgumentCurrent = NULL;    // Pointer to current va_list argument. 
    FormatData          fd;                         // 
    int                 c = 0;                      // Temporary character.
    long                lValue = 0;                 // All known supported platforms have 'long' support in hardware. 'int' is always only 32 bits (even on 64 bit systems).
    ...
    pReadFunction16(kReadActionBegin, sizeof(*pFormat), pContext);

    while(*pFormatCurrent)
    {
        char16_t cFormat = *pFormatCurrent;
        ...
        if(Isspace(cFormat))
        {
            ...
            while(Isspace((char16_t)(c = pReadFunction16(kReadActionRead, 0, pContext))))
                ++nReadCountSum;
        ...
        if(cFormat != '%')
        {
            if((c = pReadFunction16(kReadActionRead, 0, pContext)) != (int)cFormat)
            {
                pReadFunction16(kReadActionUnread, c, pContext);
                goto Done;
            }
        ...
        pFormatCurrent = ReadFormat16(pFormatCurrent, &fd);
        ...
        if((fd.mnType != 'n') && (pReadFunction16(kReadActionGetLastError, 0, pContext) || pReadFunction16(kReadActionGetAtEnd, 0, pContext)))
            break;
        ...
        switch (fd.mnType)
        {
            ...
            case 'X':
            {
                ...
                switch (fd.mModifier)
                {
                    case kModifierInt64:
                    case kModifierLongLong:
                        ullValue = (unsigned long long)ReadUint64_16(pReadFunction16, pContext, UINT64_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;

                    case kModifierInt32:
                    case kModifierLong:
                        ulValue  = (unsigned long)     ReadUint64_16(pReadFunction16, pContext, UINT32_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;

                    case kModifierInt16:
                    case kModifierShort:
                        ulValue  = (unsigned long)     ReadUint64_16(pReadFunction16, pContext, UINT16_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;

                    case kModifierInt8:
                    case kModifierChar:
                    default:
                        ulValue  = (unsigned long)     ReadUint64_16(pReadFunction16, pContext, UINT8_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;
                }
                ...
                if((fd.mnType == 'i' || fd.mnType == 'd')) // If using a signed type...
                {
                    ...
                    if(pArgumentCurrent) // If we should write the value to a user argument...
                    {
                        switch (fd.mModifier)
                        {
                            case kModifierInt64:    *(int64_t*)   pArgumentCurrent = (int64_t)   llValue; break;
                            case kModifierLongLong: *(long long*) pArgumentCurrent =             llValue; break;
                            case kModifierInt32:    *(int32_t*)   pArgumentCurrent = (int32_t)   lValue;  break; // We assume sizeof ulong >= sizeof uint32
                            case kModifierLong:     *(long*)      pArgumentCurrent = (long)      lValue;  break;
                            case kModifierInt16: 
                            case kModifierShort:    *(short*)     pArgumentCurrent = (short)     lValue;  break;
                            case kModifierInt8:  
                ...
                else
                {
                    if(bNegative)
                    {
                        ...
                        if((fd.mModifier == kModifierLongLong) || (fd.mModifier == kModifierInt64)) 
                            ...
                            ullValue = 0;  // PS2 doesn't support negating long long. Note that long long on PS2 is 128 bits and not 64 bits. So in our implementation here %I64 doesn't work on PS2.
                          #else
                            ullValue = (unsigned long long)-(long)ullValue;
                          #endif
                        else
                            ulValue  = (unsigned long)-(long)ulValue;
                    }
            ...
            case 'A':
            {
                ldValue = ReadDouble16(pReadFunction16, pContext, fd.mnWidth, fd.mDecimalPoint, nReadCount, bIntegerOverflow);
            ...
            case '[':
            {
                ...
                if(pArgumentCurrent) // If we should write the value to a user argument...
                {
                    ...
                    while(fd.mnWidth-- && ((c = pReadFunction16(kReadActionRead, 0, pContext)) != -1) && fd.mCharBitmap.Get16(c)) 
                    {
            ...
            case 'C': // Actually %C is not a standard scanf format.
            {
                ...
                if(pArgumentCurrent) // If we should write the value to a user argument...
                {
                    ...
                    while(fd.mnWidth-- && ((c = pReadFunction16(kReadActionRead, 0, pContext)) != -1))
                    {
                        switch (charTypeSize)
                        {
                            case 1:
    ...
    Done:
    if((nConversionCount == 0) && pReadFunction16(kReadActionGetLastError, 0, pContext))
        nAssignmentCount = -1;

    pReadFunction16(kReadActionEnd, 0, pContext);
...
int VscanfCore(ReadFunction32 pReadFunction32, void* pContext, const char32_t* pFormat, va_list arguments)   
{
    ...
    int                 nReadCount;                 // Temporary holder.
    int                 nReadCountSum = 0;          // Used to support the %n field.
    const char32_t*     pFormatCurrent = pFormat;   // Current position within entire fd string.
    char*               pArgumentCurrent = NULL;    // Pointer to current va_list argument. 
    FormatData          fd;                         // 
    int                 c = 0;                      // Temporary character.
    long                lValue = 0;                 // All known supported platforms have 'long' support in hardware. 'int' is always only 32 bits (even on 64 bit systems).
    ...
    pReadFunction32(kReadActionBegin, sizeof(*pFormat), pContext);

    while(*pFormatCurrent)
    {
        char32_t cFormat = *pFormatCurrent;
        ...
        if(Isspace(cFormat))
        {
            ...
            while(Isspace((char32_t)(c = pReadFunction32(kReadActionRead, 0, pContext))))
                ++nReadCountSum;
        ...
        if(cFormat != '%')
        {
            if((c = pReadFunction32(kReadActionRead, 0, pContext)) != (int)cFormat)
            {
                pReadFunction32(kReadActionUnread, c, pContext);
                goto Done;
            }
        ...
        pFormatCurrent = ReadFormat32(pFormatCurrent, &fd);
        ...
        if((fd.mnType != 'n') && (pReadFunction32(kReadActionGetLastError, 0, pContext) || pReadFunction32(kReadActionGetAtEnd, 0, pContext)))
            break;
        ...
        switch (fd.mnType)
        {
            ...
            case 'X':
            {
                ...
                switch (fd.mModifier)
                {
                    case kModifierInt64:
                    case kModifierLongLong:
                        ullValue = (unsigned long long)ReadUint64_32(pReadFunction32, pContext, UINT64_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;

                    case kModifierInt32:
                    case kModifierLong:
                        ulValue  = (unsigned long)     ReadUint64_32(pReadFunction32, pContext, UINT32_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;

                    case kModifierInt16:
                    case kModifierShort:
                        ulValue  = (unsigned long)     ReadUint64_32(pReadFunction32, pContext, UINT16_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;

                    case kModifierInt8:
                    case kModifierChar:
                    default:
                        ulValue  = (unsigned long)     ReadUint64_32(pReadFunction32, pContext, UINT8_MAX, nBase, fd.mnWidth, nReadCount, bNegative, bIntegerOverflow);
                        break;
                }
            ...
            case 'A':
            {
                ldValue = ReadDouble32(pReadFunction32, pContext, fd.mnWidth, fd.mDecimalPoint, nReadCount, bIntegerOverflow);
            ...
            case '[':
            {
                ...
                if(pArgumentCurrent) // If we should write the value to a user argument...
                {
                    ...
                    while(fd.mnWidth-- && ((c = pReadFunction32(kReadActionRead, 0, pContext)) != -1) && fd.mCharBitmap.Get32(c)) 
                    {
            ...
            case 'C': // Actually %C is not a standard scanf format.
            {
                ...
                if(pArgumentCurrent) // If we should write the value to a user argument...
                {
                    ...
                    while(fd.mnWidth-- && ((c = pReadFunction32(kReadActionRead, 0, pContext)) != -1))
                    {
                        switch (charTypeSize)
                        {
                            case 1:
    ...
    Done:
    if((nConversionCount == 0) && pReadFunction32(kReadActionGetLastError, 0, pContext))
        nAssignmentCount = -1;

    pReadFunction32(kReadActionEnd, 0, pContext);
