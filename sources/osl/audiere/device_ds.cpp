...
namespace audiere {
  ...
  DSAudioDevice::create(const ParameterList& parameters) {
    ...
    wc.lpszClassName  = L"AudiereHiddenWindow";
    RegisterClass(&wc);

    // create anonymous window
    HWND anonymous_window = CreateWindow(
      L"AudiereHiddenWindow", L"", WS_POPUP,
      0, 0, 0, 0,
	...
	if ( !guid_string.empty() ) {
		if ( UuidFromString( reinterpret_cast< unsigned short * >( const_cast< wchar_t * >( std::toWideString( guid_string ).c_str() ) ), &stack_guid ) == RPC_S_OK ) {
			guid = &stack_guid;
		}
	}
