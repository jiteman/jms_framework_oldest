...
/// 
    ...
    // The PS3 has a condition variable, which is a good primitive to 
    // use in the implementation of a thread synchronization Barrier.
    struct EABarrierData{
        sys_cond_t   mCV;             // Wait for barrier.
        sys_mutex_t  mMutex;          // Control access to barrier.
        int          mnHeight;        // Number of threads required.
        int          mnCurrent;       // Current number of threads. As threads wait, this value decreases towards zero.
        unsigned int mnCycle;         // Cycle count.
        bool         mbValid;         // True if valid.
    ...
    namespace Thread
    {
        ...
        public:
            enum Result{
                ...
                kResultSecondary =  1,  /// The barrier wait suceeded and this thread is one of hte secondary threads.
