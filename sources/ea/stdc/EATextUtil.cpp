...
#include <EAStdC/EAString.h>
    ...
    #pragma warning off (10317) // implicit arithmetic conversion from 'x' to 'y'
#endif
...
EASTDC_API char8_t* UTF8Increment(const char8_t* p, size_t n)
{
    while(n--)
    {
        ...
        if(c < 0xc2)         // Actually, any value greater than 0x80 and less than 0xc2 is an invalid leading UTF8 char.
            p += 1;
        else if(c <= 0xdf) 
            p += 2;
        else if(c <= 0xef)
