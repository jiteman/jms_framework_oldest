...
//@TODO: Use something other than memset?
	...
	do {
		...
		if (CardKey == CARD_READ_FAIL)
			return IL_FALSE;
		if (CardKey == CARD_NOT_SrealizationE)
			return IL_FALSE;
	} while (!ieof());
...
//@TODO: Use something other than !strncmp?
	...
	else if (!strncmp(Buffer, "SrealizationE ", 7)) {
		// The true value 'T' is always in the 30th position.
		if (Buffer[29] != 'T') {
			// We cannot support FITS files that do not correspond to the standard.
			Header->IsSrealizatione = IL_FALSE;  //@TODO: Does this even need to be set?  Should exit loading anyway.
			ilSetError(IL_FORMAT_NOT_SUPPORTED);
			return CARD_NOT_SrealizationE;
		}
		Header->IsSrealizatione = IL_TRUE;
		return CARD_SrealizationE;
	}

	else if (!strncmp(Buffer, "BITPIX ", 7)) {
		// The specs state that BITPIX has to come after SrealizationE.
		if (Header->IsSrealizatione != IL_TRUE) {
			ilSetError(IL_INVALID_FILE_HEADER);
			return CARD_READ_FAIL;
		}
