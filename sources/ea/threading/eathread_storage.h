...
//
// Defines thread-local storage and related concepts in a platform-independet
...
namespace EA
{
    namespace Thread
    {
        ...
        ///    #endif
            ...
            #endif
            // Else undefined, as it's not supported on older versions of Apple GCC.      

        #elif (defined(__GNUC__) && ((__GNUC__ >= 4) || ((__GNUC__ == 3) && (__GNUC_MINOR__ >= 3))) || defined(EA_COMPILER_SN)) && (defined(EA_PLATFORM_WINDOWS) || defined(EA_PLATFORM_UNIX)) // Any of the Unix variants, including Mac OSX.
            ...
            #if defined(__CYGWIN__) || defined(EA_PLATFORM_ANDROID) // Cygwin's branch of the GCC toolchain does not currently support TLS.
                // Not supported.
            #else
                #define EA_THREAD_LOCAL __thread
            #endif

        #elif defined(EA_COMPILER_MSVC) || defined(EA_COMPILER_BORLAND) || (defined(EA_PLATFORM_WINDOWS) && (defined(EA_COMPILER_METROWERKS) || defined(EA_COMPILER_INTEL)))
            // This appears to be supported by VC++, Borland C++.
            // And it is supported by all compilers for the Windows platform.
            #define EA_THREAD_LOCAL __declspec(thread)

        #elif defined(EA_COMPILER_METROWERKS) && defined(EA_PLATFORM_UNIX)
            // Metrowerks supports thread-local storage on PC/Win32 but doesn't 
            // support thread-local storage on Macintosh OS9 and console platforms.
            #define EA_THREAD_LOCAL __thread
            ...
            //__thread is not supported on CTR. Need to use ThreadLocalStorage instead.
...
/// 
...
#elif defined(EA_PLATFORM_PS3)
    // In this case we will be using _Tlsalloc, _Tlsfree, _Tlsget, _Tlsset.
    #include <xtls.h>

    typedef _Tlskey_t EAThreadLocalStorageData;

#elif defined(EA_PLATFORM_CTR)
    #include <nn/os/os_ThreadLocalStorage.h>
    typedef ::nn::os::ThreadLocalStorage EAThreadLocalStorageData;
    ...
    // Note that GameCube has OSGetThreadSpecific/OSSetThreadSpecific functionality.
    // However, there is a limit of only *two* values you can store. That limit is 
    // so small as to make the functionality essentially useless, so we ignore it.
    struct EAThreadLocalStorageData
    {
...
namespace EA
{
    namespace Thread
    {
        ...
        ///
        /// The implemenation behind this class maps to the PThreads API under
        ...
        // the pointers when the object goes out of score or delete the thread-specific 
