...
namespace Allocator {
    ...
    PPM_API size_t GetCallStack(void* pReturnAddressArray[], size_t nReturnAddressArrayLength)
    {
        ...
        if(sWin32DbgHelp.mpStackWalk)
        {
            ...
            int nStackIndex(0);
            while(nEntryIndex < (nReturnAddressArrayLength - 1))
            {
                if(!sWin32DbgHelp.mpStackWalk(IMAGE_FILE_MACHINE_I386, sWin32DbgHelp.mhCurrentProcess, hCurrentThread, &sf, &context, NULL, sWin32DbgHelp.mpSymFunctionTableAccess, sWin32DbgHelp.mpSymGetModuleBase, NULL))
                    break;
                if(sf.AddrFrame.Offset == 0)  // Basic sanity check to make sure the frame is OK. Bail if not.
                    break;
                if(nStackIndex > 0) // We ignore the first one because it is the one that is our stack walk function itself.
...
// DescribeCallStack
//
PPM_API size_t DescribeCallStack(const void* ppReturnAddressArray[], size_t nReturnAddressArrayCount, char* pBuffer, size_t nBufferLength)
{
    ...
    if(sWin32DbgHelp.mbSymInitialized)
    {
        for(size_t i(0); i < nReturnAddressArrayCount; i++)
        {
            ...
            if(sWin32DbgHelp.mpSymGetSymFromAddr && sWin32DbgHelp.mpSymGetSymFromAddr(sWin32DbgHelp.mhCurrentProcess, (DWORD)((uintptr_t)pAddress), &symDisplacement, pSymbol))
            {
                ...
                size_t nStrlen = (size_t)(unsigned)sprintf(pSymbolText, "%hs() + %d", pSymbol->Name, symDisplacement); // Possibly take the pains to get this to work with the various snprintf incarnations available.
                ...
                if(sWin32DbgHelp.mpSymGetLineFromAddr)
                {
                    ...
                    {
                        nStrlen = (size_t)(unsigned)sprintf(pSymbolText, " \"%hs\", line %d", imageHelpLine.FileName, imageHelpLine.LineNumber);
