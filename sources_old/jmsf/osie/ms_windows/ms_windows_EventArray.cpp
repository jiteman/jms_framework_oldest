...
#include "realizing/EventHandleCache.h"
...
void ms_windows_EventArray::appendEvent( const Proxy< Event > &theEvent ) throw( validating::Exception ) {
	...
	const Proxy< Iterator< Proxy< Event > > > foundEvent = assisting::find( _eventContainer, theEvent );
	_eventHandleCache->invalidate();
	_eventContainer->appendBack( theEvent );
}

void ms_windows_EventArray::appendEventArray( const Proxy< EventArray > &eventArray ) throw( validating::Exception ) {
...
void ms_windows_EventArray::setAllEvents() throw() {
	assisting::doForEvery( _eventContainer, _setEventFunctor );
}

void ms_windows_EventArray::resetAllEvents() throw() {
	assisting::doForEvery( _eventContainer, _resetEventFunctor );
}

void ms_windows_EventArray::waitForAnyEvent() const throw( validating::Exception ) {
	try {
...
const ConstantProxy< Event > ms_windows_EventArray::waitForAnyEventThenGetIt() const throw( validating::Exception ) {
...
Proxy< Event > ms_windows_EventArray::waitForAnyEventThenGetItOneself() throw( validating::Exception ) {
	Proxy< Event > event;
...
void ms_windows_EventArray::waitForAllEvents() const throw( validating::Exception ) {
...
ms_windows_EventArray::ms_windows_EventArray(
	...
	Proxy< assisting::Functor< Proxy< Event > > > setEventFunctor,
	Proxy< assisting::Functor< Proxy< Event > > > resetEventFunctor ) throw()
	:
...
const ms_windows_EventArray &ms_windows_EventArray::operator =( const ms_windows_EventArray &other ) throw( validating::Exception ) {
