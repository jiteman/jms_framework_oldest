...
namespace audiere {
  ...
  public:
    MCIDevice(const std::string& device) {
      ...
      const wchar_t* windowClassName = L"AudiereMCINotification";
  ...
  protected:
    ...
    static std::string sendString(const std::string& string, bool* error = 0, HWND window = NULL) {
      ...
      MCIERROR e = mciSendString( std::toWideString( string ).c_str(), buffer, 1000, window);

      if (error) {
        *error = (e != 0);
      }

      wchar_t errorString[ bufferLength + 1 ] = {};
      mciGetErrorString(e, errorString, bufferLength);

      if (e) {
        ADR_LOG("Error: " + std::toString( errorString ) );
      }
      if (buffer[0]) {
        ADR_LOG("Result: " + std::toString( buffer ) );
      }
      return std::toString( buffer );
    }
