...
#if defined(EA_COMPILER_MSVC) && (defined(EA_PROCESSOR_X86) || defined(EA_PROCESSOR_X86_64)) && !defined(EA_PLATFORM_AIRPLAY)
...
#ifndef EA_PLATFORM_REGISTER_SIZE // This should possibly be in EABase
    #if defined(EA_PLATFORM_XENON) || defined(EA_PLATFORM_PLAYSTATION2)
        #define EA_PLATFORM_REGISTER_SIZE 8
    #else
        #define EA_PLATFORM_REGISTER_SIZE 4
    #endif
#endif

#ifdef EA_PLATFORM_CTR
    // Supress Warning:  #61-D: integer operation result is out of range
    // when doing the operations to check if a value with overflow.
...
namespace StdC
{
    ...
    /// Example:
    ///     01010110 -> 01011001
    inline unsigned GetNextWithEqualBitCount(unsigned x){
    ...
    /// Example:
    ///        10101011 -> 10101000
    inline unsigned IsolateSingleBits(unsigned x){
        return x & ~((x << 1) | (x >> 1));
    }

    inline unsigned IsolateSingle0Bits(unsigned x){
        return IsolateSingleBits(~x);
    }

    inline unsigned IsolateSingle0And1Bits(unsigned x){
        return (x ^ (x << 1)) & (x ^ (x >> 1));
    }
    ...
    inline int32_t ShiftRightSigned(int32_t x, unsigned n){
        return int32_t((((uint32_t)x + 0x80000000u) >> n) - (0x80000000u >> n));
    }
    ...
    inline int CountTrailing0Bits(uint64_t x){
        #if defined(EA_COMPILER_MSVC) && defined(EA_PROCESSOR_X86_64) && !defined(EA_PLATFORM_AIRPLAY)
            // This has been benchmarked as significantly faster than the generic code below.
            unsigned char isNonZero;
            unsigned long index;
    ...
    inline int CountBits64(uint64_t x){ // Branchless version
        #if (EA_PLATFORM_REGISTER_SIZE < 8)
    ...
    inline uint32_t RotateLeft(uint32_t x, unsigned n){
        return (uint32_t)((x << n) | (x >> (32 - n)));
    }

    inline uint32_t RotateRight(uint32_t x, unsigned n){
        return (uint32_t)((x >> n) | (x << (32 - n)));
    }
    ...
    ///    00001000 -> true
    ///    01001110 -> false
    inline bool AreLessThan2BitsSet(int x){
        return (x & (x - 1)) == 0;
    }
    ...
    /// with no runtime cost, for any N > 0.
    ///
    ...
    inline bool UnsignedMultiplyWouldOverflow(uint32_t x, uint32_t y){
        return (y && (((x * y) / y) != x));
    }

    inline bool SignedMultiplyWouldOverflow(int32_t x, int32_t y){
        return ((y < 0) && (x == (int32_t)0x80000000)) || (y && (((x * y) / y) != x));
    }
    ...
    ///    5 / 4 -> false
    ///    3 / 0 -> true
    inline bool UnsignedDivisionWouldOverflow(uint32_t /*x*/, uint32_t y){
        return y == 0;
    }
    ...
    inline bool SignedDivisionWouldOverflow(int32_t x, int32_t y){
        return (y == 0) || ((x == (int32_t)0x80000000) && (y == -1));
    }
    ...
    ///    -3, -4 -> -4
    ///    -4,  5 ->  0
    inline int GetAverage(int x, int y){
    ...
    ///    -3, -4 -> -3
    ///    -4,  5 ->  1
    inline int GetAverage_Ceiling(int x, int y){
        return (x | y) - ((x ^ y) >> 1); // Need to use '>> 1' instead of '/ 2'
    }
    ...
    ///    }
    inline bool IsBetween0AndValue(int x, int a){
        return (unsigned)x < (unsigned)a;
    }
    ...
    /// This version doesn't work for input of 0x80000000. 
    ...
    // Alternative implementation which validates that the type is indeed an array type.
    #ifndef EAArrayCount
        #define EAArrayCount(x) (                                                   \
                    0 * sizeof( reinterpret_cast<const ::EAArrayCountTest*>(x) ) +  \
                    0 * sizeof( ::EAArrayCountTest::check_type((x), &(x))      ) +  \
                    sizeof(x) / sizeof((x)[0])  )                                  
        
        class EAArrayCountTest
        {
        public:
