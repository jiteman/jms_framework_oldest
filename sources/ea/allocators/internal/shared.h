...
namespace EA
{
    namespace Allocator
    {
        ...
        // PPM_MAY_ALIAS
        //
        #if defined(__GNUC__) && (((__GNUC__ * 100) + __GNUC_MINOR__) >= 303) && !defined(__SNC__) && !defined(EA_PLATFORM_CTR) // SNC doesn't fully support __may_alias__
        ...
        #elif defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_REVOLUTION) || defined(EA_PLATFORM_PS3_SPU)
            PPM_API int PPM_Snprintf(char* pBuffer, size_t nBufferSize, const char* pFormat, ...);
        #else
        ...
        enum DefaultFillValue
        {
                ...
                kDefaultFillValueFree        = 0x9f,  /// Fill value for user memory that has been freed.
                kDefaultFillValueDelayedFree = 0x9d,  /// Fill value for user memory which is in delayed free state.
                kDefaultFillValueNew         = 0x9b,  /// Fill value for newly allocated user memory.
                kDefaultFillValueGuard       = 0x99,  /// Fill value for guard memory returned in new user memory.
                kDefaultFillValueUnusedCore  = 0x97   /// Fill value for core that is unassigned to free or allocated memory.
            #elif defined(EA_PLATFORM_PS3)
                kDefaultFillValueDisabled    = 0x01,  /// Fill value which means to disable a given fill. Doesn't apply to guard fills or delayed free fills, which are controlled by the guard and delayed free settings.
                kDefaultFillValueFree        = 0xfe,  /// Fill value for user memory that has been freed.
                kDefaultFillValueDelayedFree = 0xfd,  /// Fill value for user memory which is in delayed free state.
                kDefaultFillValueNew         = 0xfb,  /// Fill value for newly allocated user memory.
                kDefaultFillValueGuard       = 0xf9,  /// Fill value for guard memory returned in new user memory.
                kDefaultFillValueUnusedCore  = 0xf7   /// Fill value for core that is unassigned to free or allocated memory.
            #else
                kDefaultFillValueDisabled    = 0x01,  /// Fill value which means to disable a given fill. Doesn't apply to guard fills or delayed free fills, which are controlled by the guard and delayed free settings.
        ...
        /// data output.
        PPM_API size_t DescribeCallStack(const void* pReturnAddressArray[], size_t nReturnAddressArrayLength, char* pBuffer, size_t nBufferLength);
        ...
        /// The default system page size that is assumed.
            ...
            const unsigned kDefaultMMapPageSize = 65536; // This is the page size we allocate, though the PS3 supports 1MB page sizes.
        #else
