...
/// header file.
/// 
#if (defined(EA_PLATFORM_UNIX) || EA_POSIX_THREADS_AVAILABLE) && EA_THREADS_AVAILABLE
...
#elif defined(EA_PLATFORM_PS3) && EA_THREADS_AVAILABLE
    // Condition variables are built into the PS.
    // However, the PS3 implements condition variables in a slightly unusual way.
    // While we are lobbying Sony to fix this, we implement a questionable workaround
    // in order to allow us to keep the same EAThread::Condition API, at least for 
    // the time being.
    ...
    struct EAConditionData
    {
        EA::Thread::AtomicInt32 mnInitValue; // 0 means not initialized, 1 means initializing, 2 means initialized.
        bool        mbIntraProcess;
        sys_cond_t  mCV;
        char        mName[16]; 
...
#elif defined(EA_PLATFORM_PSP2) && EA_THREADS_AVAILABLE && 0
    // We follow PS3's strategy
    #include <eathread/eathread_atomic.h>
    ...
    struct EAConditionData
    {
        ...
        EA::Thread::AtomicInt32 mnInitValue;
        bool                    mbIntraProcess;
        SceUID                  mCV;
        char                    mName[16];
    ...
    // Note that GameCube has a condition variable primitive. However, that primitive
    // doesn't have a timeout mechanism and our specification calls for one. But we 
    // already have a known-good condition variable mechanism in place here which 
    // can handle timeouts, so we use that instead of the GameCube condition variable.
    // We could probably improve on the implementation here by using some interrupt
    // disabling code on GameCube for brief sections, but that's a minor detail.
...
namespace EA
{
    namespace Thread
    {
        ...
        struct EATHREADLIB_API ConditionParameters
        {
            ...
            char mName[16];      /// Condition name, applicable only to platforms that recognize named synchronization objects.
