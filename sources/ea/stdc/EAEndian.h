...
#if defined(_MSC_VER) && (_MSC_VER >= 1300) && !defined(EA_PLATFORM_AIRPLAY)// VC++ >= v7.1
...
namespace EA
{
    namespace StdC
    {
        ...
        // MSVC7 / any platform
        #if defined(EA_PLATFORM_MICROSOFT) && (_MSC_VER >= 1300)
        ...
        /*
        9/24/2010 James Fairweather.
        CTR_TODO: Use the generic implementations of Swizzle
        until these implementations have been fully tested.
            inline uint16_t Swizzle(uint16_t x)
            {
                int temp;
                __asm {
        ...
        #elif defined(EA_PROCESSOR_X86) && (defined(EA_COMPILER_MSVC) || defined(EA_COMPILER_BORLANDC))
            inline uint16_t Swizzle(uint16_t x)
            {
                __asm xor eax, eax
                __asm mov ax, x
        ...
        // Metrowerks/PowerPC
        #elif defined(EA_PROCESSOR_POWERPC) && defined(EA_COMPILER_METROWERKS)
            inline uint16_t Swizzle(uint16_t x)
            {
                return (uint16_t)__lhbrx(&x, 0);
            }
            inline int16_t Swizzle(int16_t x)
                { return (int16_t)Swizzle((uint16_t)x); }
            #define EA_SWIZZLE_16_DEFINED
        ...
        // RVCT/ARM (ARM is a handheld device processor)
        // RVCT is the EDG-based ARM compiler.
        ...
        #elif defined(EA_PROCESSOR_ARM) && defined(EA_COMPILER_RVCT)

            // There is no __builtin_bswap16. So we just use the default C implementation, 
            // which is likely pretty fast since it's just two bytes.
        ...
        inline uint16_t SwizzleConst(uint16_t x)
        {
            return (uint16_t) ((x >> 8) | (x << 8));
        }
        inline int16_t SwizzleConst(int16_t x)
            { return (int16_t)SwizzleConst((uint16_t)x); }

        inline uint32_t SwizzleConst(uint32_t x)
        {
            return (uint32_t)
                ((x >> 24)               |
                ((x << 24) & 0xff000000) |
        ...
        inline int32_t SwizzleConst(int32_t x)
            { return (int32_t)SwizzleConst((uint32_t)x); }

        inline uint64_t SwizzleConst(uint64_t x)
        {
            const uint32_t high32Bits = Swizzle((uint32_t)(x));
            const uint32_t low32Bits  = Swizzle((uint32_t)(x >> 32));
        ...
        inline int64_t SwizzleConst(int64_t x)
