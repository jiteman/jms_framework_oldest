...
bool CallbackManager::StartThread()
{
        ...
        if(mbAsync)
        {
            if(mbThreadStarted.SetValueConditional(1, 0)) // If the thread was previously 0 and we set it to 1...
            {
                ...
                params.mpName = "CallbackManager";  // Some platforms have an extremely limited thread name buffer and will clip this.
                EA::Thread::ThreadId threadId = mThread.Begin(RunStatic, static_cast<CallbackManager*>(this), &params);
...
intptr_t CallbackManager::Run()
{
        ...
        while(mbRunning)
        {
            ...
            {
                mNSecPerTick                 = double(curTime - mNSecPerTickLastTimeMeasured) / double(curTick - mNSecPerTickLastTickMeasured);
