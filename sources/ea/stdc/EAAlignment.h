...
// The default HasTrivialDestructor template implementation doesn't always work on all compilers for vector intrinsic
// types.  It is more reliable for us to use the built in compiler traits.  It's possible we may want to do this for
// other compilers as well. Currently this is only supported on newer versions of GCC and Clang.
...
/// 
        ...
        #define EAAlignOf(type) __alignof(type)
    #elif defined(__GNUC__) || defined(__MWERKS__)
        #define EAAlignOf(type) __alignof__(type)
    #else
        ...
        #define EAAlignOf(type) ((size_t)EAAlignof3<type>::value)
    #endif
#endif
...
namespace StdC
{
    ...
    {
        #if defined(EA_PLATFORM_PLAYSTATION2) || defined(EA_PLATFORM_PSP2)
            //lint -e{923} disable lint warning about cast from pointer to size_t.
            if(((uintptr_t)p & 1) == 0) // If properly aligned...
                return *((const uint16_t*)p);
            else
            {
                unsigned char* const p8 = (unsigned char*)p;
                return p8[0] | (p8[1] << 8);
            }
        #else
            return *((const uint16_t*)p);
    ...
    inline uint32_t ReadMisalignedUint32(const void* p)
    {
        #if defined(EA_PLATFORM_PLAYSTATION2) || defined(EA_PLATFORM_PSP2)
            //lint -e{923} disable lint warning about cast from pointer to size_t.
            if(((uintptr_t)p & 3) == 0) // If properly aligned...
    ...
    inline uint64_t ReadMisalignedUint64(const void* p)
    {
        #if defined(EA_PLATFORM_PLAYSTATION2) || defined(EA_PLATFORM_PSP2)
            //lint -e{923} disable lint warning about cast from pointer to size_t.
            if(((uintptr_t)p & 7) == 0) // If properly aligned...
                return *((const uint64_t*)p);
            else
            {
                unsigned char* const p8 = (unsigned char*)p;
                return ((uint64_t)p8[0])       | ((uint64_t)p8[1] <<  8) | ((uint64_t)p8[2] << 16) | ((uint64_t)p8[3] << 24) | 
                       ((uint64_t)p8[4] << 32) | ((uint64_t)p8[5] << 40) | ((uint64_t)p8[6] << 48) | ((uint64_t)p8[7] << 56);
            }
        #else
            return *((const uint64_t*)p);
        #endif
    ...
    inline void WriteMisalignedUint16(uint16_t n, void* p)
    {
            ...
            {
                const uint8_t* const pSource8 = (const uint8_t*)&n;
                      uint8_t* const pDest8   = (uint8_t*)p;
                pDest8[0] = pSource8[0];
                pDest8[1] = pSource8[1];
            }
        #else
