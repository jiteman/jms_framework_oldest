...
#include <stdio.h> /* needed for error reporting */
...
struct malloc_chunk {
	size_t size; // Size in bytes, including overhead. Or'ed with INUSE if in use.
...
struct malloc_bin {
	...
	size_t dirty; // True if maybe consolidatable. Wasting a word here makes sizeof(bin) a power of 2, which makes size2bin() faster
};
...
static inline size_t request2size( size_t request ) {
	return
		( request == 0 ) ?
			MINSIZE :
			( ( request + MALLOC_MIN_OVERHEAD + MALLOC_ALIGN_MASK ) & ~( MALLOC_ALIGN_MASK ) );
}
