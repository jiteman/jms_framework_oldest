...
/////////////////////////////////////////////////////////////////////////
    ...
    EA_DISABLE_VC_WARNING(4265 4365 4836 4571 4625 4626 4628 4193 4127 4548 4350)
    ...
    struct EAThreadDynamicData
    {
       ...
       ~EAThreadDynamicData();
        ...
        volatile int            mnStatus;
        intptr_t                mnReturnValue;
        bool                    mbFunctionOrRunnable;   // True if function, false if runnable
        void*                   mpStartContext[2];
        void*                   mpBeginThreadUserWrapper;
        EA::Thread::AtomicInt32 mnRefCount;
        char                    mName[EATHREAD_NAME_SIZE];
        void*                   mpDynamicDataAddress;   // The address of this struct
    };
    ...
    struct EAThreadDynamicData
    {
       ...
       ~EAThreadDynamicData();
        ...
        bool                    mbAttached;                     // Denotes if we need to detach from the thread or join with it.
        void*                   mpStack;
        void*                   mpAllocatedStack;
        void*                   mpBeginThreadUserWrapper;       // User-specified BeginThread function wrapper or class wrapper
        EA::Thread::AtomicInt32 mnRefCount;
        EA::Thread::Semaphore   mThreadEndSignal;
        char                    mName[EATHREAD_NAME_SIZE];
    };
...
#elif defined(EA_PLATFORM_PLAYSTATION2) || defined(EA_PLATFORM_PLAYSTATION2_IOP) || defined(EA_PLATFORM_PSP)
    ...
    struct EAThreadDynamicData
    {
       ...
       ~EAThreadDynamicData();
        ...
        nn::os::Thread*         mThreadId; // Points to above mThread or is NULL to indicate invalidity.
        nn::os::StackMemory     mStackMemory;
    #else
        OSThread                mThread;
        OSThread*               mThreadId; // Points to above mThread or is NULL to indicate invalidity.
    #endif
        int                     mnStatus;
        intptr_t                mnReturnValue;
        void*                   mpStartContext[2];
...
namespace EA
{
    namespace Thread
    {
        ...
        struct EATHREADLIB_API ThreadParameters
        {
            ...
            int         mnProcessor;  /// 0-based index of which processor to run the thread on. A value of -1 means to use default. Default is -1.
            bool        mbSuspended;  /// If true, the thread is created initially suspended. Default is false.
...
#if defined(EA_PLATFORM_CTR)
        /// FindThreadByOsId
        /// Utility functionality to find the ThreadID by the OS ID.
        ThreadId FindThreadByOsId(bit32  Id);
#endif
    } // namespace Thread
